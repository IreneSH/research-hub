...
function renderEarnCard(e) {
  ...
  // metrics 可選：沒填就不顯示那格；都沒填就不顯示整個 metrics 區塊
  const m = e.metrics || {};
  const hasStock = m.stock_reaction && String(m.stock_reaction).trim();
  const hasRev = m.rev_yoy && String(m.rev_yoy).trim();
  let metricsHtml = '';
  if (hasStock || hasRev) {
    let cells = '';
    if (hasStock) {
      cells += `
        <div class="metric">
          ...
          <div class="metric-value ${m.stock_reaction_dir || 'neutral'}">${escapeHtml(m.stock_reaction)}</div>
        </div>`;
    }
    if (hasRev) {
      cells += `
        <div class="metric">
          ...
          <div class="metric-value ${m.rev_yoy_dir || 'neutral'}">${escapeHtml(m.rev_yoy)}</div>
        </div>`;
    }
    metricsHtml = `<div class="earn-metrics">${cells}</div>`;
  }
    ...
    <a class="earn-card" href="${escapeHtml(e.file)}">
      <div class="earn-top">
        <div class="earn-ticker-wrap">
          ...
          <div class="earn-company">${escapeHtml(e.company_en)}${e.company_zh ? ' · ' + escapeHtml(e.company_zh) : ''}</div>
        </div>
...
function renderCompanyGroup(g) {
  const itemsHtml = g.items.map(e => {
    ...
    const stockHtml = hasStock
      ? `<span class="stock-react ${m.stock_reaction_dir || 'neutral'}">${escapeHtml(m.stock_reaction)}</span>`
      : '';
    return `
  ...
  return `
    <div class="company-group">
      <div class="company-group-header">
        ...
        <span class="company-group-name">${escapeHtml(g.company_en)}${g.company_zh ? ' · ' + escapeHtml(g.company_zh) : ''}</span>
